...
```{r}
# Dropping the County and Community columns and State and Fold"
...
corr_matrix6 <- cor(CrimeData[88:101], use = "pairwise.complete.obs")
...
### 5.1 Using Poisson Regression on Our Data

```{R}
# We needed to use poisson regression on our data in order to make sure that the values made sense when we ran our regression. Since our data is non-integers, we scaled our data by multiplying it by 100 before we ran the model.
```

```{R}
# CrimeData_scaled <- as.data.frame(lapply(CrimeData, function(x) x*100))
# poissonmodel <- glm(ViolentCrimesPerPop ~ ., data = CrimeData_scaled, family=poisson())
```
...
### 5.2 Addressing Multicollinearity
...
```{R}
# We decided to use the first 12 PC's which explain about 81% of the total variance
pca_loadings <- loadings(pca)

pca_loadings_subset <- pca_loadings[, 1:12]
pca_loadings_subset
...
PC7: The seventh principal component is more associated with those who are old and retired and less associative with those who are younger with worse jobs and worse living conditions for children.
...
PC8: The eighth principal component is more associated with kids who have Moms who work and less associative with those older houses with lots of people living in them.

- Highest Loadings: Percentage of White People, percentage of those 16 and over employed in manufacturing, percentage of moms of kids 6 and under in labor force, percentage of moms of kids under 18 in labor, percent of housing occupied.     

- Lowest Loadings: Percentage of African American, mean persons per rental of household, median year house was built.      
...
fit <- lm(ViolentCrimesPerPop ~ ., data = pca_scores[, c(1:12, ncol(pca_scores))])
summary(fit)
