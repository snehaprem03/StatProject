...
### 1.4 Dealing with Variable Types
...
```{r}
# Initial Correlation Map to Visualize Initial Relationships
# This creates a correlation map with all the predictors, which is obviously too much. We will need to either combine predictors or only use select predictors for this to be readable.

library(corrplot)
# Compute the correlation matrix
corr_matrix <- cor(CrimeData[3:8], use = "pairwise.complete.obs")

# Plot the correlation matrix with color scale and significance levels
corrplot(corr_matrix, method = "color", type = "upper", order = "hclust", sig.level = 0.05, insig = "blank")
...
Here we are testing for multicollinearity in our data. Any data that has a value greater than 5 creates the risk of multicollinearity. To avoid this, we will need to remove columns that are very similar in their predictive strength. This may cause our R-squared value to decrease; however, this is necessary for the integrity of our results.
...
Here we can see that many of our variables are skewed to various degrees. We want to address this before building our regression model

```{r}
# Addressing the Skewness of the Data
